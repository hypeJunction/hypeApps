...
final class MigrationRegressionTest extends TestCase
{
    ...
    public function testNoRemovedStaticMimeType(): void
    {
        ...
        foreach (self::files('', 'php') as $file) {
            ...
            // A class is free to declare its own detectMimeType(); when it does, a `$this->detectMimeType()`
            // call binds to that method and has nothing to do with the core method removed in 4.x. Only
            // `$this->` is exempted — a call on any other receiver may still land on an \ElggFile.
            $declares_own = (bool) preg_match('/function\s+detectMimeType\s*\(/i', $src);
            $re = $declares_own
                ? '/ElggFile::detectMimeType\s*\(|(?<!\$this)->detectMimeType\s*\(/'
